...
class FlowToxicityFeatures:
    ...
    def update(self, tick: Tick) -> dict:
        ...
        realized_spread = self._compute_realized_spread(mid)
    ...
    def _compute_realized_spread(self, mid_now: float) -> float:
        """Realized spread proxy (ADV-05): effective spread NET of the price-impact
        (adverse-selection) component, measured to the CURRENT mid.

        Feedable in real time (no future prices): for a recent buy at price p with
        mid m_t, the "informed" component is the move from m_t to the current mid —
        if price rose after the buy, part of the effective spread was information,
        not cost. realized = max(0, effective - impact) per trade.
        """
        if not self._recent_trades or mid_now <= 0:
            return 0.0
        recent = list(self._recent_trades)[-50:]
        realized = []
        for t in recent:
            if t.mid <= 0:
                continue
            side_sign = 1.0 if t.side == "buy" else -1.0
            es = 2 * (t.price - t.mid) * side_sign / t.mid * 10_000
            impact = 2 * (mid_now - t.mid) * side_sign / t.mid * 10_000
            realized.append(max(0.0, es - impact))
