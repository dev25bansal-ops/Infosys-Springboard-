...
class CrossSymbolFeatures:
    ...
    def __init__(self, window_ms: int = 60_000, max_lag_ms: int = 2_000) -> None:
        ...
        self._ref_last_mid: Dict[str, float] = {}

    def update_reference(self, symbol: str, timestamp_ms: int, mid_price: float) -> None:
        """Update the reference symbol's log-return history (ENH-02).

        The reference history must store LOG RETURNS, exactly like the self
        history — never raw mids. Previously it stored the raw mid price in
        ``ret``, so cross-symbol correlation / cointegration regressed
        self-log-returns against reference-raw-mids (mismatched units).
        """
        if symbol not in self._ref_history:
            self._ref_history[symbol] = deque(maxlen=10_000)
            self._ref_last_mid[symbol] = 0.0
        hist = self._ref_history[symbol]
        last = self._ref_last_mid.get(symbol, 0.0)
        ret = np.log(mid_price / last) if last and last > 0 else 0.0
        self._ref_last_mid[symbol] = mid_price
        hist.append(_ReturnSample(timestamp_ms, ret))
