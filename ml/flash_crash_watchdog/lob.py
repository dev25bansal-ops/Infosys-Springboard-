...
class OrderBookReconstructor:
    ...
    def update_level(self, side: str, price: float, size: float) -> None:
        ...
        TST-04: unknown sides are REJECTED (warn + ignore), not silently routed
        to the ask book — a typo like "bidss"/"BUY" previously corrupted the book.
        """
        s = (side or "").lower()
        if s == "bid":
            book = self._bids
            ...
            logger.warning("update_level: unknown side %r ignored (would corrupt the book)", side)
            return
        if size == 0:
            book.pop(price, None)
        else:
