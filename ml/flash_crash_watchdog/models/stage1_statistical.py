...
class Stage1Statistical:
    ...
    def score(self, tick: Tick) -> tuple[float, bool]:
        ...
        # Leave-one-out z-scores: score this tick against a baseline that EXCLUDES
        # it. (Previously the tick was appended first, so during LUNA's slow bleed
        # the abnormal regime was absorbed into its own baseline and the z-score
        # dropped below threshold — a real crash-miss cause.)
